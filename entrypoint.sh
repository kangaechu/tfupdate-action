...
function run_tfupdate {
  case ${INPUT_RESOURCE} in
    ...
    provider)
      ...
      VERSION=$(tfupdate release list terraform-providers/terraform-provider-${INPUT_PROVIDER_NAME}| grep -v ".*-rc[0-9]$"| tail -1)
    ...
    module)
      ...
      VERSION=$(tfupdate release list --source-type=${INPUT_SOURCE_TYPE} ${INPUT_MODULE_NAME}| grep -v ".*-rc[0-9]$"| tail -1)
